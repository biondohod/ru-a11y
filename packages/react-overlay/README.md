...
│  │  ♿ Доступность                          [3] [✕]  │   │
│  │  ──────────────────────────────────────────────  │   │
│  │  ▼ ВОСПРИНИМАЕМОСТЬ                   [2] [1⚠]  │   │
│  │    ● Изображение без alt-текста  critical         │   │
│  │      img.hero-image                               │   │
│  │    ● Недостаточный контраст текста  serious       │   │
│  │      .card-subtitle                               │   │
│  │  ▼ УПРАВЛЯЕМОСТЬ                              [1] │   │
│  │    ● Кнопка без доступного названия               │   │
│  │      button.icon-btn                              │   │
│  │  ──────────────────────────────────────────────  │   │
│  │  Проверено: 14:23:05        ru-a11y-toolkit       │   │
│  └──────────────────────────────────────────────────┘   │
│                                                          │
│                              [♿ Доступность  3 ↗]       │
└─────────────────────────────────────────────────────────┘
...
|------|-----|--------------|----------|
| `excludeSelector` | `string` | `'[data-ru-a11y-overlay]'` | CSS-селектор элементов, исключаемых из сканирования |
| `highlightAll` | `boolean` | `false` | Подсвечивать все нарушения одновременно (не только выбранное) |
| `axeTags` | `string[]` | `['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa']` | Теги axe-core для запуска |
...
- [ru-a11y-toolkit](https://github.com/biondohod/ru-a11y) — монорепозиторий
