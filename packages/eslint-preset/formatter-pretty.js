...
function extractNormativeRefs(text) {
  const refs = [];
  const patterns = [
    /\[([^[\]]*?(?:ГОСТ|Постановление|WCAG)[^[\]]*)\]/g,
    /\(([^()]*?(?:ГОСТ|Постановление|WCAG)[^()]*)\)/g,
  ];
  ...
  patterns.forEach((pattern) => {
    cleanedText = cleanedText.replace(pattern, (_, refText) => {
      const normalizedRefText = normalizeWhitespace(refText);
